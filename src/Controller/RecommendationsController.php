...
{
    /**
     * @Route("/{_locale}/recommendations", name="recommendations")
     */
    public function recommendations(Request $request)
    {
        ...
        if($locale === null || !in_array($locale, $locales)) {
            return $this->redirectToRoute('recommendations', array('_locale' => $locales[0]));
        }
        if(!$this->getUser()) {
        ...
        foreach($locales as $l) {
            $translatedRoutes[] = array(
                'lang' => $l,
                'url' => $this->generateUrl('recommendations', array('_locale' => $l)),
                'active' => $l === $locale
            );
        }
